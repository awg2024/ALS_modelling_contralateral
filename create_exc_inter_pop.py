...
class create_exc_inter_population():
    def __init__(self, neuron_type):
        ...
        if neuron_type == "V2a_L":

            self.exc_inter_tonic = nest.Create('aeif_cond_beta_aeif_cond_beta_nestml',netparams.v2a_tonic_pop_size_L,self.tonic_neuronparams)

            #Create spike detectors (for recording spikes)
            self.spike_detector_exc_inter_tonic = nest.Create("spike_recorder",netparams.v2a_tonic_pop_size_L)       

        elif neuron_type == "V2a_R":

            self.exc_inter_tonic = nest.Create('aeif_cond_beta_aeif_cond_beta_nestml',netparams.v2a_tonic_pop_size_R,self.tonic_neuronparams)

            #Create spike detectors (for recording spikes)
            self.spike_detector_exc_inter_tonic = nest.Create("spike_recorder",netparams.v2a_tonic_pop_size_R)       
