...
def order_by_phase(convolved_activity, population_mean_activity, pop_name, avg_period_mean_activity):
    ...
    if generate_plot:
        ...
        true_count, true_bins = count, bins = np.histogram(phase_dist, bins=25,density=False)
        count, bins = np.histogram(phase_dist, bins=25,density=True)
        # Calculate the percentage of bins with count 0
        percentage_zero_count_bins = (np.count_nonzero(count == 0)/25)*100
        print('Num bins, % bins with 0 neurons: ',np.count_nonzero(count == 0),', ',percentage_zero_count_bins,'%')
        count_normalized = count / float(np.max(count))
        pyplot.bar(bins[:-1], count_normalized, width=np.diff(bins))
        print('The bin with the smallest count in',pop_name,' is (norm, true): ',round(np.min(count),3),np.min(true_count))
