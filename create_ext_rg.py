...
class create_rg_population():
    def __init__(self):
        ...
        self.bursting_neuronparams = {
            ...
            # aeif_cond_alpha uses single time constants
        ...
        self.rg_exc_bursting = nest.Create('aeif_cond_alpha',netparams.ext_exc_bursting_count,self.bursting_neuronparams) 
        neuron_ids = self.rg_exc_bursting.tolist() 
        print('Neuron ID (RG Ext)',neuron_ids[0],neuron_ids[-1])
        self.rg_inh_bursting = nest.Create('aeif_cond_alpha',netparams.ext_inh_bursting_count,self.bursting_neuronparams) 
        if netparams.ext_exc_tonic_count != 0: self.rg_exc_tonic = nest.Create('aeif_cond_alpha',netparams.ext_exc_tonic_count,self.tonic_neuronparams) 	
        if netparams.ext_inh_tonic_count != 0: self.rg_inh_tonic = nest.Create('aeif_cond_alpha',netparams.ext_inh_tonic_count,self.tonic_neuronparams) 
