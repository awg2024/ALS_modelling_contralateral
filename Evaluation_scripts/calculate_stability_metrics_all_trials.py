...
def analyze_output(input_1, input_2, pop_type, y_line_bd, y_line_phase, min_dist, num_motor_neurons):
    ...
    try:
        ...
        return [np.nan] * 11   # return 11 NaNs so the script keeps running
...
def find_csv_files(folder):
    ...
    mnp1_files = glob.glob(os.path.join(folder, '**', 'Figures', 'output_mnp1.csv'), recursive=True)
    mnp2_files = glob.glob(os.path.join(folder, '**', 'Figures', 'output_mnp2.csv'), recursive=True)
